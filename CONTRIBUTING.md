...
3. Run `swift test`, the strict-concurrency build documented in the workflow, and an iOS build.
   Bindgen changes must also pass `cargo fmt --check`, `cargo clippy -- -D warnings`, `cargo test`, `Tools/ic-candid-swift-bindgen/scripts/verify-artifact-bundle.sh`, the package under `Tests/BindgenPluginFixture`, and the Xcode project under `Tests/BindgenXcodePluginFixture`.
