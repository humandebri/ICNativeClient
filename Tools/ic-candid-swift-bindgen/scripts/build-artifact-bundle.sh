...
bindgen_version=$(sed -n 's/^version = "\([^"]*\)"/\1/p' "$crate_directory/Cargo.toml" | head -n 1)
...
install -d \
  "$bundle/ic-candid-swift-bindgen-$bindgen_version-macos-arm64/bin" \
  "$bundle/ic-candid-swift-bindgen-$bindgen_version-macos-x86_64/bin"

install -m 755 \
  "$crate_directory/target/aarch64-apple-darwin/release/ic-candid-swift-bindgen" \
  "$bundle/ic-candid-swift-bindgen-$bindgen_version-macos-arm64/bin/ic-candid-swift-bindgen"
install -m 755 \
  "$crate_directory/target/x86_64-apple-darwin/release/ic-candid-swift-bindgen" \
  "$bundle/ic-candid-swift-bindgen-$bindgen_version-macos-x86_64/bin/ic-candid-swift-bindgen"
