...
impl<'a> Lowerer<'a> {
    ...
    fn lower_type(&mut self, ty: &Type, suggested_name: &str) -> Result<SwiftType> {
        match ty.as_ref() {
            ...
            TypeInner::Float32 => {
                anyhow::bail!("{suggested_name}: float32 is unsupported in bindgen 0.1.3")
            }
            TypeInner::Float64 => {
                anyhow::bail!("{suggested_name}: float64 is unsupported in bindgen 0.1.3")
            }
            TypeInner::Reserved => {
                anyhow::bail!("{suggested_name}: reserved is unsupported in bindgen 0.1.3")
            }
            TypeInner::Empty => {
                anyhow::bail!("{suggested_name}: empty is unsupported in bindgen 0.1.3")
            }
            TypeInner::Func(_) => {
                anyhow::bail!("{suggested_name}: func values are unsupported in bindgen 0.1.3")
            }
            TypeInner::Service(_) | TypeInner::Class(_, _) => {
                anyhow::bail!("{suggested_name}: service values are unsupported in bindgen 0.1.3")
            }
            TypeInner::Knot(_) => {
        ...
        for definition in self.definitions.values() {
            ...
            for dependency in reachable {
                ...
                if self.reaches(&dependency, &definition.name, &mut BTreeSet::new()) {
                    anyhow::bail!(
                        "mutually recursive types {} and {} are unsupported in bindgen 0.1.3",
                        definition.name,
                        dependency
                    );
                }
